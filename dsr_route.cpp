...
int DsrRoutePacket::parseRouteFromBuf(const char* reqRouteBuf)
{
    uint32_t* cur = (uint32_t*)reqRouteBuf;

    int tmpReqID = ntoh32(*cur);

    // TODO: 此处应当抛出异常，强制调用者try catch
    if (tmpReqID != this->reqID) {
        cout << "Request ID not match!" << endl;
        return -1;
    }
...
int DsrRoutePacket::serializeRouteToBuf(char* reqRouteBuf)
{
    uint32_t* cur = (uint32_t*)reqRouteBuf;
